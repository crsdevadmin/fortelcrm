# backend/import_mahesh.py
# Imports Mahesh C's doctor list (94 records)
# Run from fortel-crm folder: python -m backend.import_mahesh
...
EXCEL_PATH = os.path.join(os.path.dirname(__file__), '..', 'client6032731961782296870.xls')
# If not there, adjust:
# EXCEL_PATH = r"C:\Users\mjaff\Downloads\client6032731961782296870.xls"

db = SessionLocal()

mahesh = db.query(User).filter(User.email == 'mahesh.c@fortel.in').first()
if not mahesh:
    print("ERROR: mahesh.c@fortel.in not found. Run create_new_users.py first.")
    db.close()
    sys.exit(1)
print(f"Mapping to: {mahesh.name} (id={mahesh.id})")
...
for i in range(1, ws.nrows):
    ...
    doc = Doctor(
        ...
        status=val(row,24) or 'Active', manager_id=mahesh.id, is_active=True,
    )
    db.add(doc)
    created += 1
