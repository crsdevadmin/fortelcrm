# backend/import_suresh.py
# Imports Suresh Kamalraj K's doctor list from the uploaded Excel
# Run from fortel-crm folder: python -m backend.import_suresh
...
# ── Path to the uploaded Excel ──────────────────────────────────
EXCEL_PATH = os.path.join(os.path.dirname(__file__), '..', 'client10583524841782296746.xls')

# If file not found at default path, adjust here:
# EXCEL_PATH = r"C:\Users\mjaff\Downloads\client10583524841782296746.xls"

db = SessionLocal()

# Find Suresh
suresh = db.query(User).filter(User.email == 'suresh.k@fortel.in').first()
if not suresh:
    print("ERROR: suresh.k@fortel.in not found. Run create_new_users.py first.")
    db.close()
    sys.exit(1)
print(f"Mapping to: {suresh.name} (id={suresh.id})")
