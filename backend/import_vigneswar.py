# backend/import_vigneswar.py
# Imports Vigneswar M's doctor list (127 records — Kerala)
# Run from fortel-crm folder: python -m backend.import_vigneswar
...
EXCEL_PATH = os.path.join(os.path.dirname(__file__), '..', 'client18025629031782297035.xls')
# If not found, adjust:
# EXCEL_PATH = r"C:\Users\mjaff\Downloads\client18025629031782297035.xls"

db = SessionLocal()

vigneswar = db.query(User).filter(User.email == 'vigneswar.m@fortel.in').first()
if not vigneswar:
    print("ERROR: vigneswar.m@fortel.in not found. Run create_new_users.py first.")
    db.close()
    sys.exit(1)
print(f"Mapping to: {vigneswar.name} (id={vigneswar.id})")
...
for i in range(1, ws.nrows):
    ...
    doc = Doctor(
        ...
        status=val(row,24) or 'Active', manager_id=vigneswar.id, is_active=True,
    )
    db.add(doc)
    created += 1
