...
export default function WeeklyReports() {
  ...
  const statusColors = { green: '#047857', amber: '#b45309', red: '#b91c1c', unassigned: '#64748b' };
  const statusBgs = { green: '#ecfdf5', amber: '#fffbeb', red: '#fef2f2', unassigned: '#f1f5f9' };
  const trendDelta = metric => summary.trends?.[metric]?.change_pct ?? deltaPct(summary[metric], priorSummary[metric]);
    ...
    <div style={{ minHeight: '100vh', background: '#f3f4f6', padding: '22px 24px 42px' }}>
      ...
      {loading ? <div style={{ padding: 60, textAlign: 'center', color: '#64748b' }}>Generating and saving weekly report…</div> : error ? (
        ...
        <>
          ...
          {report.is_partial && <div style={{ marginBottom: 12, padding: '10px 13px', borderRadius: 10, background: '#fffbeb', border: '1px solid #fde68a', color: '#92400e', fontSize: 10, fontWeight: 700 }}>Partial report — covers through {report.covers_through}. It will refresh automatically as the week progresses.</div>}
          {(report.data_warnings || []).map(warning => <div key={warning.code} style={{ marginBottom: 12, padding: '10px 13px', borderRadius: 10, background: '#fff7ed', border: '1px solid #fed7aa', color: '#9a3412', fontSize: 10 }}>{warning.message} {warning.value ? `Value: ${fmtInr(warning.value)}` : ''}</div>)}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(175px,1fr))', gap: 10, marginBottom: 18 }}>
            <SummaryCard label="Regional Sales" value={fmtInr(summary.regional_week)} detail={`MTD ${fmtInr(summary.regional_mtd)} · ${fmtInr(summary.regional_sales_per_day)}/day`} accent="#2563eb" delta={trendDelta('regional_week')} />
            <SummaryCard label="Doctor-wise Sales" value={fmtInr(summary.doctor_week)} detail={`MTD ${fmtInr(summary.doctor_mtd)} · ${fmtInr(summary.doctor_sales_per_day)}/day`} accent="#047857" delta={trendDelta('doctor_week')} />
            <SummaryCard label="Investment" value={fmtInr(summary.investment_week)} detail={`Entered this week · ROI ${summary.roi_week == null ? 'N/A' : `${summary.roi_week}x`}`} accent="#c2410c" delta={trendDelta('investment_week')} />
            <SummaryCard label="Doctor Visits" value={summary.visits_week || 0} detail="Visits this week" accent="#7c3aed" delta={trendDelta('visits_week')} />
            ...
            <SummaryCard label="Required Actions" value={summary.actions || 0} detail={`${fmtInr(summary.value_at_risk)} quantified gap · ${summary.status_counts?.red || 0} red`} accent="#b91c1c" />
          </div>

          {(report.movers || []).length > 0 && <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: '13px 15px', marginBottom: 18 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginBottom: 8 }}><span style={{ fontSize: 13, fontWeight: 900 }}>Biggest Score Changes</span>{summary.best_week_last_12 && <span style={{ fontSize: 9, color: '#047857', background: '#ecfdf5', padding: '3px 8px', borderRadius: 12, fontWeight: 800 }}>Best total-sales week in last 12 snapshots</span>}</div><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>{report.movers.map(item => <span key={item.user_id} style={{ fontSize: 10, border: '1px solid #e5e7eb', borderRadius: 20, padding: '5px 9px', color: item.change >= 0 ? '#047857' : '#b91c1c' }}><strong>{item.name}</strong> {item.change >= 0 ? '+' : ''}{item.change} points</span>)}</div></div>}
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, overflow: 'hidden', marginBottom: 18 }}>
            ...
            <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 920, borderCollapse: 'collapse' }}>
              ...
              <tbody>{(report.people || []).map((person, index) => <tr key={person.user_id} style={{ borderTop: '1px solid #f1f5f9' }}>
                <td style={{ padding: 10, fontSize: 10, color: '#94a3b8' }}>{person.score == null ? '—' : index + 1}</td><td style={{ padding: 10 }}><div style={{ fontSize: 10, fontWeight: 900 }}>{person.name}</div><div style={{ fontSize: 8, color: '#94a3b8' }}>{person.display_role}</div></td><td style={{ padding: 10, fontSize: 13, fontWeight: 900, color: statusColors[person.status] }}>{person.score == null ? 'N/A' : person.score}</td><td style={{ padding: 10 }}><span style={{ fontSize: 8, fontWeight: 900, color: statusColors[person.status], background: statusBgs[person.status], borderRadius: 12, padding: '3px 7px' }}>{person.status.toUpperCase()}</span></td><td style={{ padding: 10, fontSize: 10 }}>{person.doctor_sales_pct == null ? 'N/A' : `${person.doctor_sales_pct}%`}</td><td style={{ padding: 10, fontSize: 10 }}>{person.regional_sales_pct == null ? 'N/A' : `${person.regional_sales_pct}%`}</td><td style={{ padding: 10, fontSize: 10 }}>{person.recovery_pct == null ? 'N/A' : `${person.recovery_pct}%`}</td><td style={{ padding: 10, fontSize: 10 }}>{person.visit_coverage_pct == null ? 'N/A' : `${person.visit_coverage_pct}%`}</td><td style={{ padding: 10, fontSize: 10 }}>{person.weekly_score == null ? 'N/A' : `${person.weekly_score}%`}</td><td style={{ padding: 10, fontSize: 10 }}>{person.task_score == null ? 'N/A' : `${person.task_score}%`}</td>
              </tr>)}</tbody>
            </table></div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 16 }}>
            <div style={{ background: '#fff', border: '1px solid #fecaca', borderRadius: 14, padding: '14px 16px' }}><div style={{ fontSize: 13, fontWeight: 900, color: '#991b1b', marginBottom: 10 }}>Management Actions · highest financial gap first</div>{(report.actions || []).length ? report.actions.slice(0, 15).map((action, index) => <div key={`${action.person}-${action.reason}-${index}`} style={{ padding: '7px 0', borderTop: '1px solid #fee2e2', display: 'flex', justifyContent: 'space-between', gap: 10 }}><span style={{ fontSize: 10 }}><strong>{action.person}</strong> · {action.reason}</span><span style={{ textAlign: 'right' }}>{Number(action.gap_value) > 0 && <strong style={{ display: 'block', color: '#991b1b', fontSize: 10 }}>{action.gap_unit === 'currency' ? fmtInr(action.gap_value) : `${action.gap_value} item${Number(action.gap_value) === 1 ? '' : 's'}`}</strong>}<span style={{ fontSize: 8, fontWeight: 900, color: statusColors[action.status] }}>{action.status.toUpperCase()}</span></span></div>) : <div style={{ color: '#047857', fontSize: 11 }}>No immediate actions</div>}</div>
